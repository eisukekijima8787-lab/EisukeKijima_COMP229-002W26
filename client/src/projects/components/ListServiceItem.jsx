...
const ListservicesItem = ({ services }) => {
  ...
  return (
    <li style={{ padding: '10px 0', borderBottom: '1px solid #ddd', color: 'black', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <div>
        <span style={{ fontWeight: 'bold' }}>📁 {services.servicesName || services.title}</span>
        <span style={{ marginLeft: '10px', color: 'brown' }}>Status: {services.status}</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center' }}>
        {/* --- Editボタン: 編集画面へ飛ばす --- */}
        <Link 
          to={`/services/edit/${services._id}`} 
  ...
  if (window.confirm("Are you sure you want to delete this service?")) {
    axios.delete(`http://localhost:3004/api/services/${services.id}`, {
      headers: { 
    ...
    .catch(err => {
      ...
      alert(`Failed to delete service. Reason: ${err.response?.status} (${err.response?.data?.message || 'Unauthorized'})`);
              });
            }
          }}
        >
